...
class PosSessionInherit(models.Model):
    ...
    def get_closing_control_data(self):
        ...
        #     })
        last_cash_move = self.sudo().statement_line_ids.sorted('create_date')[-1] if self.sudo().statement_line_ids else 0
        if last_cash_move == 0:
            name = 'Opening Balance'


        elif last_cash_move.amount > 0:
        ...
        if last_cash_move != 0:
            cash_in_out_list.append({
                'name': last_cash_move.payment_ref if last_cash_move !=0 else name,
                'amount': last_cash_move.amount if last_cash_move!=0 else 0,
                'is_new_currency_selected': last_cash_move.is_new_currency_selected if last_cash_move!=0 else False,
            })
