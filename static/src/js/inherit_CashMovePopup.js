odoo.define('pos_custome.InheritCashMovePopup', function (require) {
    ...
    class InheritCashMovePopup extends CashMovePopup {
        setup() {
            super.setup();
            this.state = useState({
                inputComment: '',
        ...
        active_currency() {
            console.log("complementary_currency", this.env.pos);
            return [
                { 'name': this.env.pos.get_order().pos.currency.name },
                { 'name': this.env.pos.config.complementary_currency[1] }
            ];
        }
